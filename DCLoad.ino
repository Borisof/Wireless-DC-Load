...
void setup(void) {
	...
	pinMode(BT_SW, INPUT);
	pinMode(Display_SW, INPUT);
	pinMode(BT_EN, OUTPUT);
	digitalWrite(BT_EN, LOW);
	
	ADC1.begin();
	DAC1.setPortWrite(true);
	DAC1.output(0);	//Makes sure current is set to zero upon power up
...
void ReadSwitches()
{
	...
	int readingDispSW = digitalRead(Display_SW);
...
void ChangeDisplay()
{
	...
	{
		LCD.Clear();
		digitalWrite(DisplayBL, DisplaySate);
		Bluetooth.flush();
	}
	else
	{
...
void Bluetooth_RX()
{
	...
	if (Bluetooth.available())
	{
		...
		if (BTBuffer[i - 1] == '\0' && BTBuffer[0] == '*' && BTBuffer[1] == '~')
			...
			case 'S':DisplaySate = !DisplaySate; ChangeDisplay(); break;	// Turn display off/on		
